...
- A primeira, $$\tau = \mathbb{E}[\mu_1(X)]- \mathbb{E}[\mu_0(X)],$$em que $\mu_i(X) = \mathbb{E}[Y(i) | X] = \mathbb{E}[Y | Z = i, X]$.
- A segunda, $$\tau = \mathbb{E}\Big[\dfrac{ZY}{e(X)} \Big] - \mathbb{E}\Big[\dfrac{(1- Z)Y}{1 - e(X)} \Big],$$em que $e(X) = P(Z = 1 | X)$ é o _propensity score_.

## Introdução


- A primeira abordagem, precisa ajustar um modelo para o resultado dado o tratamento e covariáveis. É consistente se o modelo utilizado está corretamente especificado. 
- A segunda abordagem precisa ajustar um modelo para o tratamento dadas as covariáveis. É consistente se o modelo do _propensity score_ for corretamente especificado.
...
- Postulemos um modelo para a média condicional do resultado, $\mu_1(X, \beta_1)$ e $\mu_0(X, \beta_0)$.
...
- Note que os termos do meio, aumentam $\mu_i(X, \beta_i)$ pelos residuais ponderados pelo _inverse propsensity score_.
- Os termos finais, aumentam o IPW pelos resultados imputados.
...
::: {.callout-tip}
### Estimador
Baseados nos dados observados $\{X_i, Z_i, Y_i\}_{i = 1}^n$, podemos obter um estimador DR para $\tau$ através dos seguintes passos:
...
::: {.callout-note}
### Implementação
Implementaremos os quatro métodos vistos até agora. O modelo utilizado para o _propsensity score_ será o modelo logístico.
...
OS_est <- function(z, y, x, out_family = gaussian, alphas = c(0, 1)) {
  ...
  res_1 <- y - outcome_1
  res_2 <- y - outcome_0
  tau_dr <- mean(z * res_1 / e) + mean((1 - z) * res_2 / (1 - e)) + tau_reg
...
```{r}
#| echo: true
OS_ATE <- function(z, y, x, n_boot = 500, out_family = gaussian, alphas = c(0, 1)) {
...
for (i in 1:mc) {
  ...
  beta_z <- c(0, 1, 1)
  e <- 1 / (1 + exp(-as.vector(x %*% beta_z)))
  z <- rbinom(n, 1, e)
  beta_y1 <- c(1, 2, 1)
  beta_y0 <- c(1, 2, 1)
  y1 <- rnorm(n, x %*% beta_y1)
  y0 <- rnorm(n, x %*% beta_y0)
  y <- z * y1 + (1 - z) * y0 
  ce <- OS_ATE(z, y, x)
  bias[i,] <-  ce[1, ] - 0
  estim_se[i, ] <- ce[2, ]
