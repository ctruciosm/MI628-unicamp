...
subtitle: "CRE: Abordagem de Neyman"
...
- Neyman (1923) propõe um método para fazer inferência para o efeito causal médio (ACE: **A**verage **C**ausal **E**ffect).
...
> Estamos interessados em estimar o efeito causal médio (ACE), $\tau$, baseados em $(Z_i, Y_i)_{i = 1}^n$ sob CRE.
...
Utilizando os resultados observados ($Y_i$), podemos calcular aa médiaa e variâncias amostrais como:
...
[Nenhuma!. Não existe versões amostrais de $S(1, 0)$ nem $S^2(\tau)$ pois os resultados potenciais $Y_i(1)$ e $Y_i(0)$ nunca são observados simultaneamente para cada unidade $i$.]{style="color:red;"}
...
- Repare que apenas $Z_1, \cdots, Z_n$ é aleatório (mas os resultados potenciais são todos fixos).
...
- No FRT, todos os $T(\textbf{z}^m, \textbf{Y})$ são calculáveis, já na abordagem de Neyman, os $\hat{\tau}^m$ são valores hipotéticos pois não todos os resultados potenciais são conhecidos (apenas conhecemos os resultados observados).
...
intervalo_confianca
var_hat_tau <- n0 * var(y1) / (n1 * n) + n1 * var(y0) / (n0 * n) + 2*cov(y1, y0) / n
var_hat_tau
```
:::
...
- O objetivo é determinar se o uso do tratamento causa algum efeito em $Y_i$.
...
Vejamo o que acontece se ajustarmos um modelo de regressão por MQO.
...
::: {.nonincremental}
- $\dfrac{\hat{\tau}-\tau}{\sqrt{\hat{V}}} = \dfrac{1794.3431}{670.9967} = 2.674146 \quad , \quad \dfrac{\hat{\tau}-\tau}{\sqrt{\hat{V}_{MQO}}} = \dfrac{1794.3431}{632.8536} = 2.835321$
- $\dfrac{\hat{\tau}-\tau}{\sqrt{\hat{V}_{EHW}}} = \dfrac{1794.3431}{669.3155} = 2.680863 \quad , \quad \dfrac{\hat{\tau}-\tau}{\sqrt{\hat{V}_{HC2}}} = \dfrac{1794.3431}{670.9967} = 2.674146$
:::
