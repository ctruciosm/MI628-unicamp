...
- _Propensity score_ é uma ferramenta importante na inferência causal. O foco da aula de hoje está em entender o motivo pelo qual é tão importante.
...
Ademais, \begin{align}
P(X, Z, Y(1), Y(0)) &= P(Z, Y(1), Y(0) | X) \times P(X), \\
                    &= P(Z | X, Y(1), Y(0)) \times P(Y(1), Y(0) | X) \times P(X)
\end{align}
...
<center>
$e(X)$ apresenta uma série de propriedades que serão bastante úteis para inferirmos causalidade.
</center>


<aside>
**Observação:** alguns autores definem _propensity score_ apenas como $e(X) = P(Z = 1 | X)$ seguindo a notação de Rosembaum e Rubin (1983). Essa notação é utilizada pois os autores focam em estudos observacionais sob ignorabilidade forte. A definição $e(X, Y(1), Y(0)) = P(Z = 1| X, Y(1), Y(0))$ é uma definição mais geral e em alguns casos pode ser de interesse.
</aside>
...
- Assim, temos um [SRE](https://ctruciosm.github.io/MI628-unicamp/MI628_Aula07#/popula%C3%A7%C3%A3o-finita-super-popula%C3%A7%C3%A3o-11), o que implica em $K$ CRES independentes em cada estrato do _propensity score_ e podemos analizar os dados observacionais da mesma forma como no SRE (mas agora estratificado em $e(X)$) `r emo::ji("smile")`
...
- $\hat{e}(X)$ não precisa ser discreto (de fato pode assumir tanto valores quantas observações disponíveis) `r emo::ji("sad")`.
- Contudo, podemos discretizar (utilizando os quantis ou algum outro método) $\hat{e}(X)$ e termos (aproximadamente) $$Z \perp\!\!\!\perp  \{ Y(1), Y(0)\} | \hat{e}'(X) = e_k \quad (k = 1, \cdots, K),$$ em que $\hat{e}'(X)$ são os valores discretizados `r emo::ji("smile")`.
...
<aside>
A condição $0 < e(X) < 1$ é conhecida como a condição de sobreposição (*overlap*) ou positividade (*positivity*). Ela é necessária para o Teorema estar bem definido.
</aside>
...
- Contudo, esta suposição é bastante forte (D'Amour et al. 2021) e mesmo se a condição é verificada para $e(X)$ não há garantia que seja verificada para $\hat{e}(X)$ (o que permite que o estimador _exploda_, tornando os resultados bastante instáveis).
...
::: {.callout-tip}
### Teorema
O _propensity score_ satisfaz $$Z \perp\!\!\!\perp X | e(X).$$ Ademais, para qualquer função $h(\cdot)$, temos que $$\mathbb{E} \Big [\dfrac{Zh(X)}{e(X)} \Big] = \mathbb{E} \Big[\dfrac{(1 - Z) h(X)}{1- e(X)} \Big],$$ desde que ambas as $\mathbb{E}(\cdot)$ existam.
:::
...
- Portanto, dentro de cada nível do _propensity score_, a distribuição das covariáveis é balanceada entre tratamento e controle.
