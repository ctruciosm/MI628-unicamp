...
- Seré no contexto de estudos observacionais podemos ajustar MQP $Y$ sob $(1, Z, X, Z \times X)$?
    *   **Sim!**
...
::: {.callout-tip}
### Teorema
Se $\bar{X} = 0$ and $(\mu_1(X, \hat{\beta}_1), \mu_0(X, \hat{\beta}_0)) = (\hat{\beta}_{10} + X \hat{\beta}_{11}, \hat{\beta}_{00} + X \hat{\beta}_{01})$ baseado na regressão por MQP de $Y$ sob $(1, Z, X, Z \times X)$ com $w_i = \left\{ \begin{array}{ll}
		\dfrac{1}{\hat{e}(X_i)} & \text{se }  Z_i = 1,\\
		\dfrac{1}{1 - \hat{e}(X_i)} & \text{se }  Z_i = 0.
...
|:------------------:|:----------:|:--------------------------:|
| Sem $X$            | $Y~(1, Z)$ | $Y~(1, Z)$ com pesos $w_i$ |
| Com $X$            | $Y~(1, Z, X, Z \times X)$ | $Y~(1, Z, X, Z \times X)$ com pesos $w_i$ |
