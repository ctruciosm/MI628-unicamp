...
[Correlação e causalidade podem co-existir, mas correlação não implica causalidade.]{style="color:red;"}
...
<center>
[Nesta matéria (ME920/MI626) aprenderemos diversas técnicas que nos permitirão tirar conclusões do tipo causa-efeito.]{style="color:red;"}
</center>
...
#### Inferência Causal ou Econometria?
...
em que $n_{zy} = \# \{i: Z_i = z, Y_i = y \}$. Podemos estimar RD, RR e OR, substituindo $p_zy$ por $\hat{p}_{zy} = n_{zy}/n$ com $n$ sendo o total de observações.
...
- Contudo, os dados não vem de um RCT (randomized controlled trial). Isso implica que pacientes recebendo o tratamento 1 podem ser bem diferentes do que pacientes recebendo o tratamento 0.
...
- Sejam **A** e $Y$, duas variáveis aleatorias binárias: **A** (1: tratamento, 0: controle) e $Y$ (1: morte, 0: sobrevive).
- Seja $Y^{a = 1}$ a variável resultado que teria sido observada se $a = 1$ e seja $Y^{a = 0}$ a variável resultado que teria sido observada se $a = 0$.
- Juan tem $Y^{a = 1} = 1$ e $Y^{a = 0} = 0$.
- Maria tem $Y^{a = 1} = 0$ e $Y^{a = 0} = 0$
...
O tratamento **A** tem um efeito causal no resultado de um indivíduo $Y$ se $$Y^{a = 1} \neq Y^{a = 0}.$$
:::


## Definições básicas

As variáveis $Y^{a = 1}$ e $Y^{a = 0}$ são conhecidas como **potential outcomes** ou **counterfactual outcomes**. Aquela que corresponde ao valor do tratamento que o indivíduo de fato recebeu é chamada de **factual**.



. . . 

Para definir o efeito causal de um indivíduo precisamos:

::: {.nonincremental}

1. Um resultado de interesse, 
2. As ações $a = 1$ e $a = 0$ a ser comparadas, e
3. O indivíduo cujos resultados contrafactuais (_counterfactual outcomes_) $Y^{a = 0}$ d $Y^{a = 1}$ serão comparados.
...
Identificar efeitos causais individuais não é geralmente possível. Assim, devemos olhar para o  **efeito causal médio** numa população de indivíduos.


. . . 


Para definir o efeito causal médio, precisamos de:
...
1. Um resultado de interesse, 
2. As ações $a = 1$ e $a = 0$ a ser comparadas, e
3. Uma população bem definida de individuos cujos resultados $Y^{a = 0}$ e $Y^{a = 1}$ serão comparados
...
Pensemos na familia do Juan como nossa população de interesse e vejamos os resultados contrafactuais sobre $a = 1$ e $a = 0$
...
knitr::kable(table, col.names = c("$Y^{a = 0}$", "$Y^{a = 1}$"), escape = FALSE, row.names = TRUE) %>% kable_styling(font_size = 16, full_width = TRUE)
```
...
- [Quantos elementos da população teriam morto se tivessem recebido o transplante ($Y^{a = 1} = 1$)?]{style="color:red;"}
- $Pr[Y^{a = 1} = 1] = 10/20 = 0.5$
- [Quantos elementos da população teriam morto se não tivessem recebido o transplante ($Y^{a = 0} = 1$)?]{style="color:red;"}
...
O efeito causal médio do tratamento A sobre o resultado $Y$ esta presente se $$Pr[Y^{a = 1} = 1] \neq Pr[Y^{a = 0} = 1],$$

ou, equivalentemente, se $$\mathbb{E}(Y^{a = 1}) \neq \mathbb{E}(Y^{a = a}),$$ em que a última definição pode ser utilizada seja $Y$ binária ou não.
...
<center>
[Ausência do efeito causal média não implica ausência do efeito causal individual.]{style="color:red;"}
</center>


. . . 

Como o efeito causal médio é comumente chamado de **efeito causal** e a hipótese nula de **não efeito médio causal** é chamada simplesmente de **hipótese causal nula**.


## Definições básicas


Note que a ausência (ou não) do efeito causal média pode também ser medida através de:

- $Pr(Y^{a = 1} = 1) - Pr(Y^{a = 0} = 1) = 0$
- $\dfrac{Pr(Y^{a = 1} = 1)}{Pr(Y^{a = 0} = 1)} = 1$
- $\dfrac{Pr(Y^{a = 1} = 1) \big / Pr(Y^{a = 1} = 0)}{Pr(Y^{a = 0} = 1) \big / Pr(Y^{a = 0} = 0)} = 1$


## Definições básicas

- Na prática, nunca temos temos a Tabela como no exemplo da familia do Juan (ou observamos $Y^{a = 1}$ ou $Y^{a = 0}$, nunca ambos).
- Na prática, observamos o nível do tratamento ($A$) e o resultado obtido ($Y$).
...
Quando a proporção de indivíduos que $Y = 1$ no grupo de tratamento e no de controle são iguais, ou seja, $$P(Y = 1 | A = 1) = P(Y = 1 | A = 0),$$
...
- $\dfrac{P(Y = 1 | A = 1)}{P(Y = 1 | A = 0)} = 1$
- $\dfrac{P(Y = 1 | A = 1) \big / P(Y = 0| A = 1)}{P(Y = 1 | A = 0) \big / P(Y = 0| A = 0)} = 1$
...
<center>
![Fonte: "Causal Infernce: What if?"](imagens/what_if.png)
</center>
