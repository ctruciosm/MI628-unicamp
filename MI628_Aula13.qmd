...
- No caso ideal, teremos um _macth_ perfeito.
...
Ou seja, condicionado nas covariáveis e no fato de sabermos que em cada par temos um tratamento e um controle, temos um MPE!

. . . 


<center>
**Podemos analisar _match_ perfeitos em estudos observacionais como se fossem um MPE!**
</center>
...
- O _index_ $ij$ representa a unidade $j$ no conjunto $i$ ($i = 1, \cdots, n$, $j = 1, \cdots, M_i$) com indicadora de tratamento $Z_{ij}$ e resultados potenciais $Y_{ij}(1)$ e $Y_{ij}(0)$
...
Pode-se mostrar que $$\hat{\tau}^{mbc} = \hat{\tau}^{Reg} + \dfrac{1}{n}\displaystyle \sum_{i = 1}^n \Big[ \Big( 1 + \dfrac{K_i}{M} \Big ) Z_i \hat{R_i}  - \Big( 1 + \dfrac{K_i}{M} \Big )(1 - Z_i) \hat{R_i}\Big ]$$
